// FIXME:: Failed to Loaded database from file
...
void Database_load(struct Connection *conn) {
  ...
  if (!(conn->db && conn->file)) {
    diec("Invalid Connection Info", conn);
  }
  // First: load (int)max_rows & (int)max_data
  if (fread(&conn->db->max_rows, sizeof(conn->db->max_data), 1, conn->file) !=
      1) {
  ...
  if (!conn->db->rows)
    diec("Failed to loaded database's rows", conn);

  // Third: load every struct Address
  for (size_t i = 0; i < conn->db->max_rows; ++i) {
    // 1. Load (int)id
    if (fread(&conn->db->rows[i].id, sizeof(conn->db->rows[i].id), 1,
              conn->file) != 1) {
      diec("Failed to load database element's id", conn);
    }
    // 2. Load (int)set
    if (fread(&conn->db->rows[i].set, sizeof(conn->db->rows[i].set), 1,
              conn->file) != 1) {
      diec("Failed to load database element's set", conn);
    }
    // 3. Load (char[max_data])name
    if (fread(&conn->db->rows[i].name, sizeof(char) * conn->db->max_data, 1,
              conn->file) != 1) {
      diec("Failed to load database element's name", conn);
    }
    // 4. Load (char[max_data])email
    if (fread(&conn->db->rows[i].email, sizeof(char) * conn->db->max_data, 1,
              conn->file) != 1) {
...
void Database_write(struct Connection *conn) {
  ...
  if (fwrite(&conn->db->max_rows, sizeof(conn->db->max_rows), 1, conn->file) !=
      1) {
    diec("Failed to write MAX_ROWS of Database", conn);
  }
  if (fwrite(&conn->db->max_data, sizeof(conn->db->max_data), 1, conn->file) !=
      1) {
    diec("Failed to write MAX_DATA of Database", conn);
  }

  for (size_t i = 0; i < conn->db->max_rows; ++i) {
    if (fwrite(&conn->db->rows[i].id, sizeof(conn->db->rows[i].id), 1,
               conn->file) != 1) {
      diec("Failed to write id of Address", conn);
    }
    if (fwrite(&conn->db->rows[i].set, sizeof(conn->db->rows[i].set), 1,
               conn->file) != 1) {
      diec("Failed to write set of Address", conn);
    }
    if (fwrite(&conn->db->rows[i].name, sizeof(char) * conn->db->max_data, 1,
               conn->file) != 1) {
      diec("Failed to write name of Address", conn);
    }
    if (fwrite(&conn->db->rows[i].email, sizeof(char) * conn->db->max_data, 1,
               conn->file) != 1) {
      diec("Failed to write email of Address", conn);
    }
  }

  int rc = fflush(conn->file);
  if (rc == -1) {
    diec("Cannot flush database.", conn);
  }
}
...
void Database_create(struct Connection *conn) {
  ...
  if (conn->db->max_rows <= 0)
    diec("MAX_ROWS must be positive.", conn);

  printf("MAX_DATA: ");
  scanf("%d", &conn->db->max_data);
  if (conn->db->max_data <= 0)
    diec("MAX_DATA must be positive.", conn);
  ...
  for (int i = 0; i < conn->db->max_rows; ++i) {
    ...
    conn->db->rows[i].name =
        (char *)memset(conn->db->rows[i].name, 0, conn->db->max_data);
    conn->db->rows[i].email = (char *)malloc(sizeof(char) * conn->db->max_data);
    conn->db->rows[i].email =
        (char *)memset(conn->db->rows[i].email, 0, conn->db->max_data);
  }
}
